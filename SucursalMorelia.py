...
with open('credentialsMore.json') as file:
    credentialsMore = json.load(file)
    user = credentialsMore["credentials"][0]["user"]
    password = credentialsMore["credentials"][0]["password"]
    host = credentialsMore["credentials"][0]["host"]
    port = credentialsMore["credentials"][0]["port"]
    nameDB = credentialsMore["credentials"][0]["database"]
try:
...
#--------------------------Archivo de las tablas adicionales---------------#
'''def cargarTablas(num):
    with open('tablas.txt', 'r') as f:
        x = f.readlines()
    stripped_line_of_the_temps = [s.rstrip() for s in x]
    d1=eval(stripped_line_of_the_temps[num])
    nombretablas=[]
    atributostablas=[]

    for k, v in d1.items():
        nombretablas.append(k)
        atributostablas.append(v)
    return (nombretablas,atributostablas)'''
#--------------------------Archivo de las tablas adicionales---------------#
...
def MenuDirecciones():
    print("\nMenu de direcciones de Morelia:")
...
print("\n*-*-*-*-*-*-*-*-*-*-*-*-*Bienvenido a la Sucursal Morelia*-*-*-*-*-*-*-*-*-*-*-*\n")
...
opcionBDMorelia = input("Dime con un numero la opcion a elegir: ")
        
if opcionBDMorelia == "1":
    MenuClientes()
elif opcionBDMorelia == "2":
    MenuDirecciones()
elif opcionBDMorelia == "3":
    MenuCompras()
elif opcionBDMorelia == "4":
    crearTabla()
